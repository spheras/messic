    ...
    implements DAOAuthor
{
    ...
    {
        Query query =
            entityManager.createQuery( "from MDOAuthor as a where (1=1 AND a.owner.login= :userName) order by rand()" );
    ...
    public List<String> getFirstCharacters( String username )
    {
        ...
        Query query =
            entityManager.createQuery( "select DISTINCT UPPER(SUBSTRING(name, 1,1)) as letter from MDOAuthor as a where (a.owner.login= :userName) order by letter" );
        query.setParameter( "userName", username );
        @SuppressWarnings( "unchecked" )
    ...
    {
        Query query =
            entityManager.createQuery( "from MDOAuthor as a where (a.owner.login = :userName)  ORDER BY UPPER(a.name)" );
        query.setParameter( "userName", username );
    ...
    {
        Query query =
            entityManager.createQuery( "from MDOAuthor as a where (a.owner.allowDLNA = true)  ORDER BY UPPER(a.name)" );
    ...
    {
        Query query =
            entityManager.createQuery( "from MDOAuthor as a where (a.owner.login = :userName) AND (a.sid = :authorSid)" );
        query.setParameter( "userName", username );
    ...
    {
        Query query =
            entityManager.createQuery( "from MDOAuthor as a where (UPPER(a.name) LIKE :authorName) AND (a.owner.login = :userName)  ORDER BY UPPER(a.name)" );
    ...
    {
        Query query =
            entityManager.createQuery( "from MDOAuthor as a where (UPPER(a.name) = :authorName) AND (a.owner.login = :userName)" );
        query.setParameter( "userName", username );
