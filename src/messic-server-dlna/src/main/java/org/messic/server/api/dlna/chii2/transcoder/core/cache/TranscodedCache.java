...
public class TranscodedCache
{
    ...
    private Map<String, TranscoderProcess> processCache =
        Collections.synchronizedMap( new LinkedHashMap<String, TranscoderProcess>() );

    // Transcoded Process Timer Cache
    private Map<String, ScheduledFuture> timerCache =
        Collections.synchronizedMap( new LinkedHashMap<String, ScheduledFuture>() );
    ...
    // Logger
    private Logger logger = Logger.getLogger( TranscodedCache.class );
        ...
        if ( process != null )
        {
            ...
            if ( process.isStarted() && !process.isFinished() && !process.isStopped() )
            {
                ...
                if ( !processCache.containsKey( id ) )
                {
                    ...
                    logger.debug( "Add transcoded process from request {} to cache." );
                    timerCache.put( id, this.createProcessTimer( id ) );
                    logger.info( "Add transcoded process timer from request {} to cache." );
                    // Remove old one
                    if ( processCache.size() > MAX_PROCESS_COUNT )
                    {
                        for ( Map.Entry<String, TranscoderProcess> entry : processCache.entrySet() )
                        {
                            ...
                            logger.info( "Remove transcoded process from request {} from cache because cache limited." );
                            // Cancel Timer
                            ...
                            logger.info( "Timer from request {} has been canceled." );
                            // Terminate process
                            logger.info( "Terminate process from request {} because cache limited." );
            ...
            else
            {
                ...
                if ( process.isStarted() && process.isFinished() )
                {
                    ...
                    if ( !fileCache.containsKey( id ) )
                    {
                        ...
                        // logger.debug("Add transcoded file {} from request {} to cache.",
                        // process.getOutputFile().getAbsolutePath(), id);
                        // Remove old one
                        if ( fileCache.size() > MAX_FILE_COUNT )
                        {
                            for ( Map.Entry<String, File> entry : fileCache.entrySet() )
                            {
                                ...
                                // logger.info("Remove transcoded file {} from request {} from cache because cache limited.",
                                // file.getAbsolutePath(), entry.getKey());
                                // Delete file
                                // logger.info("Delete transcoded file {} from request {}.", file.getAbsolutePath(),
                                // entry.getKey());
    ...
    /**
     * Shutdown the cache This should be called before application exit
     */
    ...
    private void deleteFile( File file )
    {
        if ( file != null && file.exists() )
        {
            ...
            {
                // logger.error("Try to delete temp file {} with Security Exception: {}", file.getAbsolutePath(),
                // e.getMessage());
            }
        }
    }
