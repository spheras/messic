...
public class AuthorController
{
    ...
    @ApiMethod( path = "/services/authors/{authorSid}/cover", verb = ApiVerb.GET, description = "Get cover for a certain author", produces = { MediaType.IMAGE_JPEG_VALUE } )
    @ApiErrors( apierrors = { @ApiError( code = UnknownMessicRESTException.VALUE, description = "Unknown error" ),
        @ApiError( code = NotAuthorizedMessicRESTException.VALUE, description = "Forbidden access" ),
    ...
    @RequestMapping( value = "/{authorSid}/cover", method = RequestMethod.GET )
    @ResponseStatus( HttpStatus.OK )
    @ResponseBody
    @ApiResponseObject
    public ResponseEntity<byte[]> getAuthorCover( @PathVariable @ApiParam( name = "authorSid", description = "SID of the author to get the cover", paramType = ApiParamType.PATH, required = true ) Long authorSid,
        ...
        IOMessicRESTException
    {
        ...
        try
        {
            byte[] content = authorAPI.getAuthorCover( user, authorSid, preferredWidth, preferredHeight );
